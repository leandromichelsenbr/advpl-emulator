...
O projeto interpreta um subconjunto controlado de AdvPL e não executa código arbitrário. Consultas ao banco Protheus não são executadas; exemplos que dependem de dados externos precisam fornecer dados de referência. O pré-processador suporta `#define` simples e parametrizado, `#undef`, `#ifdef`, `#ifndef`, `#else`, `#endif`, includes virtuais e subconjuntos de `#translate`/`#xtranslate` em formato de chamada ou sequência literal de palavras e identificadores, preservando strings, comentários e a origem por arquivo. A execução assíncrona procura headers conhecidos no catálogo distribuído; manifests explícitos continuam tendo prioridade. Traduções com pontuação, expressões, grupos opcionais/listas e regras `#command`/`#xcommand` são reconhecidas e informadas, mas ainda não expandidas. Ainda não estão implementados `VALID`, `WHEN`, máscaras `PICTURE`, recursos ou classes completas da LIB.
