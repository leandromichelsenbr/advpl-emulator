...
O projeto interpreta um subconjunto controlado de AdvPL e não executa código arbitrário. Consultas ao banco Protheus não são executadas; exemplos que dependem de dados externos precisam fornecer dados de referência. O pré-processador suporta `#define` simples e parametrizado, `#undef`, `#ifdef`, `#ifndef`, `#else`, `#endif` e includes virtuais, preservando strings, comentários e a origem por arquivo. A execução assíncrona procura headers conhecidos no catálogo distribuído; manifests explícitos continuam tendo prioridade. `#command`, `#xcommand`, `#translate` e `#xtranslate` são reconhecidos e informados, mas ainda não expandidos. Ainda não estão implementados `VALID`, `WHEN`, máscaras `PICTURE`, recursos ou classes completas da LIB.
