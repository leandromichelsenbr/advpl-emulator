...
Catálogo verificável das capacidades anunciadas pelo AdvPL Emulator. Esta matriz cobre a distribuição `0.18.0`, o perfil `protheus-default`, o contrato público `0.1` e o modelo intermediário `0.1`.
...
| LNG-003 | Pré-processamento controlado | `partial` | `#define` simples e parametrizado, `#undef`, condicionais, includes virtuais aninhados, catálogo distribuído, traduções de chamada (`NOME(<arg>)`) e sequências literais (`BYREF <nome>`), expansão segura, ciclos, diagnósticos, proveniência por arquivo, `capabilities`, `applied` e painel Fonte × PPO. | Pontuação, expressões capturadas, grupos opcionais/listas de `#translate` e regras `#command` ainda não são aplicados; sem expressões condicionais completas. | `preprocessor.test.js`, `include-loader.test.js`, regras reais de `COMMON.CH`/`APWEBSRV.CH` e mensagem multilinha com `CRLF` em `core.test.js`. |
...
| UNS-002 | Arquivos `.ch` externos | `partial` | `#include` resolve manifests explícitos ou os 103 headers do catálogo educacional distribuído. Arquivos ausentes recebem `PP0006`; não há leitura do disco ou AppServer. |
