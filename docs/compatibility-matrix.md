...
Catálogo verificável das capacidades anunciadas pelo AdvPL Emulator. Esta matriz cobre a distribuição `0.16.0`, o perfil `protheus-default`, o contrato público `0.1` e o modelo intermediário `0.1`.
...
| LNG-003 | Pré-processamento controlado | `partial` | `#define` simples e parametrizado, `#undef`, condicionais, includes virtuais aninhados, catálogo distribuído, expansão segura, ciclos, diagnósticos, proveniência por arquivo, `capabilities`, `applied` e painel Fonte × PPO. | Regras `#command`/`#translate` são reconhecidas, mas ainda não aplicadas; sem expressões condicionais completas. | `preprocessor.test.js`, `include-loader.test.js` e mensagem multilinha com `CRLF` em `core.test.js`. |
