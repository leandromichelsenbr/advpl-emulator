...
Catálogo verificável das capacidades anunciadas pelo AdvPL Emulator. Esta matriz cobre a distribuição `0.11.0`, o perfil `protheus-default`, o contrato público `0.1` e o modelo intermediário `0.1`.
...
| LNG-002 | Funções escalares e de array | `partial` | `Space`, `AllTrim`, `StrTran`, `SubStr`, `Abs`, `cValToChar`, `Chr`, `Len`, `ACopy` e `AClone`; `StrTran` cobre substituições controladas e `SubStr` cobre posição positiva/negativa, quantidade opcional e limites. | Assinaturas, tipos e erros cobrem apenas os exemplos testados; posição zero de `SubStr` não integra o contrato garantido. | `core.test.js`: exemplos de `StrTran`, `SubStr`, `If/Else`, `ACopy`, `AClone` e expressões. |
