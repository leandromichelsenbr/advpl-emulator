...
Catálogo verificável das capacidades anunciadas pelo AdvPL Emulator. Esta matriz cobre a distribuição `0.15.0`, o perfil `protheus-default`, o contrato público `0.1` e o modelo intermediário `0.1`.
...
| LNG-003 | Pré-processamento controlado | `partial` | `#define`, `#undef`, condicionais, includes virtuais aninhados, catálogo distribuído, expansão fora de strings/comentários, ciclos, diagnósticos, proveniência por arquivo, `capabilities`, `applied` e painel Fonte × PPO. | Macros com parâmetros e regras `#command`/`#translate` são reconhecidas nos headers, mas ainda não aplicadas; sem expressões condicionais completas. | `preprocessor.test.js`, `include-loader.test.js` e mensagem multilinha com `CRLF` em `core.test.js`. |
