...
test("mantém os contratos públicos na versão 0.14.0 do pacote", () => {
  assert.equal(core.VERSION, "0.1.0");
  assert.equal(core.API_VERSION, "0.1");
  assert.equal(core.PACKAGE_VERSION, "0.14.0");
...
Return`);
  ...
  assert.equal(browse.headerClickAction.toLowerCase(), 'alert(\'bheaderclick\')');
  assert.equal(browse.doubleClickMessage, "bLDblClick");
  assert.equal(browse.doubleClickAction.toLowerCase(), 'alert(\'bldblclick\')');
  assert.equal(core.parseAction(browse.headerClickAction)[0].kind, "stop");
  assert.equal(core.parseAction(browse.doubleClickAction)[0].kind, "stop");
